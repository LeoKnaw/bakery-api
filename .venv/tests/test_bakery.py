...
class TestProducts:
    ...
    def test_create_product(self, client):
        ...
        response = client.post(
            "/fadel/products", json={"name": f"{TEST_PREFIX}Bread", "price": 1000.0}
        )
    ...
    def test_get_products(self, client):
        ...
        client.post(
            "/fadel/products", json={"name": f"{TEST_PREFIX}Cake", "price": 500.0}
        )
    ...
    def test_update_product(self, client):
        ...
        create_resp = client.post(
            "/fadel/products", json={"name": f"{TEST_PREFIX}Croissant", "price": 700.0}
        )
    ...
    def test_update_nonexistent_product(self, client):
        ...
        response = client.put(
            f"/fadel/products/{fake_id}", json={"name": "Ghost", "price": 100.0}
        )
...
class TestProduction:
    ...
    def test_create_production(self, client, db_session):
        ...
        # Record production
        response = client.post(
            "/fadel/production", json={"product_id": product_id, "quantity": 50}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_create_production_nonexistent_product(self, client):
        """Test production for non-existent product returns 404"""
        fake_id = str(uuid.uuid4())
        response = client.post(
            "/fadel/production", json={"product_id": fake_id, "quantity": 10}
        )
        assert response.status_code == 404
...
class TestSales:
    ...
    def test_sale_retail(self, client, db_session):
        ...
        client.post(
            "/fadel/production", json={"product_id": product_id, "quantity": 20}
        )
    ...
    def test_sale_wholesale(self, client, db_session):
        ...
        response = client.post(
            "/sales", json={"items": [{"product_id": product_id, "quantity": 7}]}
        )
        assert response.status_code == 200

    def test_sale_supply(self, client, db_session):
    ...
    def test_sale_insufficient_stock(self, client, db_session):
        ...
        create_resp = client.post(
            "/fadel/products", json={"name": f"{TEST_PREFIX}No Stock", "price": 1000.0}
        )
        product_id = create_resp.json()["id"]

        # Try to sell more than available (0)
        response = client.post(
            "/sales", json={"items": [{"product_id": product_id, "quantity": 5}]}
        )
...
class TestInventory:
    ...
    def test_get_inventory(self, client, db_session):
        ...
        client.post(
            "/fadel/production", json={"product_id": product_id, "quantity": 30}
        )
...
class TestWholesaleClassification:
    ...
    def test_auto_classify_wholesale_at_5(self, client, db_session):
    ...
    def test_retail_below_5(self, client, db_session):
        ...
        # Sale with qty = 4 (no explicit sale_type)
        response = client.post(
            "/sales",
            json={
                "items": [
